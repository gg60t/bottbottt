...
ask_ques = "الرجاء اختيار الجلسة الخاص بك اذا كنت تريد استخراج سورس ايكثون فإختار ايكثون وأما إذا تريد سورس ميوزك اختار بايروجرام"
buttons_ques = [
    [
        InlineKeyboardButton("Pyrogram", callback_data="pyrogram1"),
        InlineKeyboardButton("ICTHON", callback_data="telethon"),
    ],
    [
    ...
    [
        ...
        InlineKeyboardButton("ICTHON Bot", callback_data="telethon_bot"),
    ],
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, old_pyro: bool = False, is_bot: bool = False):
    ...
    try:
        ...
        await api_id_msg.reply('غير صالح API_ID (which must be an integer). Please start generating session again.', quote=True, reply_markup=InlineKeyboardMarkup(Data.generate_button))
        return
    api_hash_msg = await bot.ask(user_id, 'الرجاء إرسال الخاص بك `API_HASH`', filters=filters.text)
    if await cancelled(api_hash_msg):
        return
    api_hash = api_hash_msg.text
    if not is_bot:
        t = "الرجاء إرسال الخاص بك `PHONE_NUMBER` along with the country code. \nExample : `+19876543210`'"
    else:
        t = "الرجاء إرسال `BOT_TOKEN` \nExample : `12345:abcdefghijklmnopqrstuvwxyz`'"
    ...
    if not is_bot:
        ...
        try:
            ...
            await msg.reply('انتهت صلاحية الرمز لمرة واحدة. يرجى البدء في إنشاء الجلسة مرة أخرى‌‌.', reply_markup=InlineKeyboardMarkup(Data.generate_button))
            return
            ...
            try:
                ...
                await msg.reply('لقد تعديت الوقت المحدد 5 دقائق لإستخراج الكود الخاص بك.', reply_markup=InlineKeyboardMarkup(Data.generate_button))
                return
            try:
